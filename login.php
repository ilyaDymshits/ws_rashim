...
function log_error($target, $username, $course, $reason) {
    $event = \local_ws_rashim\event\user_login_failed::create ( 
            array (
                'userid' => - 1,
                'other' => array (
                    'username' => $username,
...
$mfgs_krs = ( integer ) $_POST ['mfgs_krs']; // will set to 0 if not supplied...
$mfgs_sid = ( integer ) $_POST ['mfgs_sid'];
        ...
        if ($course != '') {
            ...
            if (isset ( $resource_type ) && ! empty ( $resource_type )) {
                ...
                if (! $courseget = $DB->get_record ( 'course', $conditions )) {
                    ...
                    if (! $meeting = $DB->get_record ( 'meetings', $conditions )) {
                        $url = "$CFG->httpswwwroot/course/view.php?id={$courseget->id}";
                    } else {
                        if (isset ( $meeting->section_id )) {
                        ...
                        $url = "$CFG->httpswwwroot/course/view.php?id={$courseget->id}&section={$meeting->section_num}";
                    }
                }
            }
        }
